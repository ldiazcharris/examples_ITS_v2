...
In red letters, some of automatically added signals are shown in Figure 7.

The RNA allocates signals close to the buffer stops. Some of the signals allocated in buffer stops are:
...
Routes 1 to 5 are the same in interlocking tables shows in Table 2 and Table 3, but RNA considers tracks as bidirectional, while the original layout has only one-direction per track.
...
Table 4 shows the additional routes, considering bidirectional tracks. Routes 6 to 10 are the opposite of routes 1 to 5. So it does not affect safety, RNA always considers every possible route in the layout. Moreover, departure signals are considered for line borders and buffer stops for extra protection.
