...
This example was mentioned in the written paper titled: "Automatic Railway Signalling Generation for Railways Systems Described on Railway Markup Language (railML)". Henceforth, when we refer to the paper, we will do it as [1].
...
After having installed the RNA program according to the steps shown in the [Link](https://github.com/GICSAFePhD/Layouts) in the section "Usage", run the Python archive "main_GUI.py". This action produces the below program output:
...
Figure 2 it can see the railway network without signalling. Although this figure will not show by the program, it is a visual help for the user of this manual example. The user will need the Rail-AID program and import the archive "Example_2.railml" to visualise the railway network for this example.
...
The result of this step is the output console:
...
In this example, it is possible to view that the program can identify the nodes and their directions, as shown in Figure 2: ne14, ne15, ne16, ne17, ne18, ne19 and ne20.
...
However, in [1] in the section "II. RAILWAY NETWORK ANALYSER DESIGN" in literal C, it can see Algorithm 2, which explains the process of analysing the network; and in the same section but in literal D, it explains the process for detecting CDL zones.
...
To obtain an analysis for each network element in the program configurations (view Figure 5), select only the option you want. Dismark all options and only keep marked what you need. Repeat this logic step for follows steps.
...
Signals are enumerated since 00 with a prefix letter to indicate which element generates them. BufferStops and LineBorders signals start with T and L, respectively. In Figure 7, we can visualise some of the signals generated by applying algorithm 3, explained in [1] section "III. SIGNALLING GENERATION".
...
Notice that RNA can be configured to avoid adding this signalling when the level crossing and the platform are close together; therefore, the signalling between them is unnecessary.

Figure 11 shows the signals (in red letters, added signals are shown):

- Generated due level crossings: *X07 and X08*.
...
To simplify signals is needed that, mark the configuration option "Simplify signals".
...
Then Rail-AID will display the table of routes for this network. It is shown in Figure 19.
